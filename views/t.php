...
if (isset($_SESSION['role']) && $_SESSION['role'] === 'auteur') {
    ...
    // Fetch articles with their associated tags
    $articles = [];
    $query = "SELECT article.id, article.title, article.description, article.date_creation, article.image, article.catagugry_id, catagugry.name AS category_name, tags.tagsname AS tag_name
              FROM article
              JOIN catagugry ON article.catagugry_id = catagugry.id
              LEFT JOIN article_tags ON article.id = article_tags.id_article
              LEFT JOIN tags ON article_tags.id_tags = tags.id";
    ...
    // Group articles by ID and collect their tags
    $groupedArticles = [];
    foreach ($articles as $article) {
        ...
        if (!isset($groupedArticles[$articleId])) {
            $groupedArticles[$articleId] = [
                ...
                'image' => $article['image'],
                'catagugry_id' => $article['catagugry_id'],
                'category_name' => $article['category_name'],
                'tags' => []
            ];
        }
        if ($article['tag_name']) {
            $groupedArticles[$articleId]['tags'][] = $article['tag_name'];
        }
    }
    ...
    die("Access denied. You must be logged in as an author.");
}
?>
    ...
    <main class="min-h-screen flex">
        ...
        <aside class="w-64 bg-gradient-to-b from-red-800 to-green-900 text-white">
            ...
            <nav class="mt-6">
                ...
                <a href="login.html" class="flex items-center px-6 py-3 hover:bg-red-700 transition-colors duration-200">
                    ...
                    <article class="relative bg-white shadow-md rounded-md">
                        ...
                        <form method="POST">
                            ...
                            <select id="tagname" name="tagsname" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                                <?php foreach ($tags as $tag): ?>
                                    <option value="<?= $tag['id'] ?>"><?= $tag['tagsname'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div id="selectedTagsContainer" class="mt-2">
                            <?php foreach ($article['tags'] as $tag): ?>
                                <span class="bg-blue-200 hover:bg-blue-300 py-1 px-2 rounded-lg text-sm"><?= htmlspecialchars($tag) ?></span>
                            <?php endforeach; ?>
                        </div>
